...
// Route Express : POST /api/gemini/analyze
// Reçoit le CV + poste, vérifie les crédits de l'utilisateur en Firestore,
// appelle Gemini côté serveur (clé secrète jamais exposée), parse le JSON,
// décrémente les crédits UNIQUEMENT après un succès réel.
...
/**
 * Construit le prompt strict envoyé à Gemini.
 * RÈGLE ABSOLUE respectée : interdiction d'inventer des informations.
 */
...
/** Nettoie une réponse Gemini qui pourrait contenir des balises Markdown autour du JSON. */
function extractJson(rawText) {
  ...
  // Si du texte reste avant/après, on isole la première accolade et la dernière.
...
router.post('/analyze', requireAuth, async (req, res) => {
  ...
  try {
    // 1. Vérifier les crédits AVANT d'appeler Gemini (contrôle serveur uniquement)
    ...
    if (!apiKey) {
      return res.status(500).json({ error: 'SERVER_MISCONFIGURED', message: 'Clé Gemini non configurée côté serveur.' });
    }
    ...
    try {
      ...
      geminiResponse = await fetch(GEMINI_ENDPOINT(GEMINI_MODEL, apiKey), {
        method: 'POST',
      ...
      if (networkErr.name === 'AbortError') {
        return res.status(504).json({ error: 'TIMEOUT', message: 'Gemini a mis trop de temps à répondre. Réessayez.' });
      }
      return res.status(503).json({ error: 'NETWORK_ERROR', message: 'Impossible de contacter Gemini. Vérifiez votre connexion.' });
    }

    if (geminiResponse.status === 429) {
      return res.status(429).json({ error: 'QUOTA_EXCEEDED', message: 'Quota Gemini gratuit dépassé. Réessayez plus tard.' });
    }
    if (geminiResponse.status === 401 || geminiResponse.status === 403) {
      return res.status(500).json({ error: 'INVALID_KEY', message: 'Clé Gemini invalide côté serveur.' });
    }
    if (!geminiResponse.ok) {
      return res.status(502).json({ error: 'GEMINI_UNAVAILABLE', message: 'Gemini est indisponible pour le moment.' });
    }
    ...
    if (!rawText) {
      return res.status(502).json({ error: 'EMPTY_RESPONSE', message: 'Gemini a renvoyé une réponse vide.' });
    }

    // 3. Parser le JSON
    let structuredCv;
    try {
      structuredCv = extractJson(rawText);
    } catch (parseErr) {
      return res.status(502).json({
        error: 'PARSE_ERROR',
        message: 'La réponse de l\'IA n\'a pas pu être analysée. Réessayez.'
      });
    }

    // 4. Décrémenter le crédit UNIQUEMENT maintenant (succès confirmé)
    await userRef.update({
      credits: admin_FieldValueIncrement(-1),
      updatedAt: new Date().toISOString()
    });

    // 5. Sauvegarder le document (optionnel, permet "Mes documents")
...
// Petit helper pour éviter d'importer FieldValue partout (évite une dépendance circulaire de style)
...
/** Appelle Gemini avec un prompt donné et retourne le JSON parsé. Lève une erreur { status, code, message }. */
